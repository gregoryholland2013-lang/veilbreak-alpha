...
import { Home, Layers, Swords, Sparkles, Zap, ScrollText, BookOpen, Mail, Grid3X3, X, Users, Shield, Flame } from 'lucide-react';
...
const primaryNav = [
  ...
  { path: '/summon', icon: Sparkles, label: 'Summon' },
];
...
const allMenuItems = [
  ...
  { path: '/summon', icon: Sparkles, label: 'Summon', desc: 'Draw new cards', color: 'text-purple-300' },
  ...
  { path: '/event', icon: Flame, label: 'Event', desc: 'Event dungeon & raids', color: 'text-orange-300' },
            ...
            >
              <div className="bg-card border border-border rounded-2xl shadow-2xl overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-border">
                  <p className="font-display font-bold text-sm text-primary">All Features</p>
                  <button onClick={() => setMenuOpen(false)} className="w-7 h-7 rounded-lg bg-muted/60 flex items-center justify-center text-muted-foreground hover:text-foreground">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                    ...
                    return (
                      <Link
                        ...
                        className={`flex items-center gap-3 rounded-xl p-3 transition-all
                          ${active ? 'bg-primary/10 border border-primary/30' : 'bg-muted/30 hover:bg-muted/60 border border-transparent'}`}
                      >
                        <div className={`w-8 h-8 rounded-lg bg-card flex items-center justify-center flex-shrink-0 ${active ? 'border border-primary/40' : ''}`}>
                          <Icon className={`w-4 h-4 ${color}`} />
                        </div>
                        <div className="min-w-0">
                          <p className={`font-display font-bold text-xs ${active ? 'text-primary' : 'text-foreground'}`}>{label}</p>
        ...
        <div className="max-w-lg mx-auto flex justify-around items-center py-1 pb-3">
          {primaryNav.map(({ path, icon: Icon, label }) => {
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 transition-all ${active ? 'drop-shadow-[0_0_8px_hsl(45,80%,55%,0.8)]' : ''}`} />
                <span className={`text-[10px] font-bold tracking-wider uppercase ${active ? 'text-primary' : ''}`}>{label}</span>
              </Link>
            );
          })}
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all duration-200
              ${menuOpen ? 'text-primary bg-primary/10' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'}`}
          >
            <Grid3X3 className={`w-5 h-5 ${menuOpen ? 'drop-shadow-[0_0_8px_hsl(45,80%,55%,0.8)]' : ''}`} />
            <span className={`text-[10px] font-bold tracking-wider uppercase ${menuOpen ? 'text-primary' : ''}`}>More</span>
          </button>
        </div>
      </nav>
    </>
